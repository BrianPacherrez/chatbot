...
class SheetManager {
    ...
    // Crear pestaña para Producto
    async ensureSheetExists(sheetTitle: string): Promise<void> {
    ...
    // Agrega esto dentro de la clase SheetManager
    async addProductEntry(number: string, product: string): Promise<void> {
        try {
            await this.ensureSheetExists('producto');
            ...
            await this.sheets.spreadsheets.values.append({
                ...
                requestBody: {
                    values: [[number, product, now]],
                },
            });

            console.log("✅ Producto guardado correctamente en la hoja 'producto'");
        } catch (error) {
            console.error("❌ Error al guardar el producto en la hoja 'producto':", error);
        }
    }
